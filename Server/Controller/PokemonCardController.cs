...
public class PokemonCardController : ControllerBase
{
    ...
    [HttpPost("set")] // Example: https://localhost:7240/api/PokemonCard/xy3?pageSize=12&pageNumber=1
    public async Task<ActionResult<PagedList<PokemonCardDataResponse>>> GetCardsBySet(
        [FromBody] PagedRequest request)
    {
        try
        {
            ...
            var cardDto = await query
                .Where(c => c.Id.Substring(0, set!.Length) == set)
            ...
            var totalCount = cardDto.Count();

            var result = new PagedList<PokemonCardDataResponse>
            {
                Data = cardDto,
                TotalCount = totalCount,
                PageNumber = pageNumber,
            ...
            _logger.LogInformation("Returning {Count} results for {setName} set.", totalCount, set);

            return Ok(result);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting cards with " + request.Set + " set.");
            throw new HttpRequestException("Error getting cards with " + request.Set + " set. (Http Request Exception)");
        }
    }
